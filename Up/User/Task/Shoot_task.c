...
static void model_choice(void)
{
    ...
    bay_control();
    // 取消注释开始发射
    if (switch_is_mid(rc_ctrl[TEMP].rc.switch_left) || switch_is_up(rc_ctrl[TEMP].rc.switch_left) || friction_flag == 1)
    {
        // 发射
        friction_control();
        dial_control();
...
static void dial_control(void)
{
    if (switch_is_up(rc_ctrl[TEMP].rc.switch_left) ||
        rc_ctrl[TEMP].mouse.press_l &&
            shooter.shoot_heat_limit > shooter.shoot_heat + 30) // 鼠标左键按下发弹
    {
        LEDR_ON();
        LEDB_OFF();
        LEDG_OFF();
        shooter.dial_speed_target = key_dial_directed_speed;
    }
    else if (rc_ctrl[TEMP].key[KEY_PRESS].f) // 按下f键，拨盘电机反转
    {
        LEDR_OFF();
        LEDB_ON();
        LEDG_OFF();
        shooter.dial_speed_target = key_dial_reverse_speed;
    }
    else // 其他情况，拨盘电机停转
    {
        LEDR_OFF();
...
static void GetKeyBoard()
{
    if (rc_ctrl[TEMP].key[KEY_PRESS].q) // 摩擦轮电机动
        ...
        friction_flag = 0;
    if (rc_ctrl[TEMP].key[KEY_PRESS].f) // 拨盘电机反转，防止卡弹
        key_dial_reverse_speed += KEY_ENTER_OFFSET;
    else
        key_dial_reverse_speed -= KEY_SLOW_OFFSET;
    if (key_dial_reverse_speed > MAX_DIAL_SPEED / 2)
        key_dial_reverse_speed = MAX_DIAL_SPEED / 2;
    else if (key_dial_reverse_speed < 0) // 防止溢出
        key_dial_reverse_speed = 0;
