...
{
    // 初始化pid参数
    gimbal_Yaw.pid_parameter[0] = 80, gimbal_Yaw.pid_parameter[1] = 0, gimbal_Yaw.pid_parameter[2] = 0;
    gimbal_Yaw.pid_angle_parameter[0] = 10, gimbal_Yaw.pid_angle_parameter[1] = 0.01, gimbal_Yaw.pid_angle_parameter[2] = 0;
    gimbal_Yaw.angle_target = 0;

    // 初始化pid结构体
    pid_init(&gimbal_Yaw.pid, gimbal_Yaw.pid_parameter, 20000, 20000);
...
static void RC_Yaw_control()
{
    if (rc_ctrl.rc.ch[0] >= -660 && rc_ctrl.rc.ch[0] <= 660)
    {
        ...
        if (rc_ctrl.mouse.press_r && is_track)
        {
